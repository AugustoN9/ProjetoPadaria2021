...
public class UsuarioDAO {
	...
	public void cadastro(Usuario usuario) {
		...
		String sql ="INSERT INTO public.usuario (nome, cpf, endereco, telefone, email, username, senha, nivel_acesso) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?)";
		try {
			PreparedStatement preparador = con.prepareStatement(sql);
			preparador.setString(1, usuario.getNome());
			preparador.setString(2, usuario.getCpf());
			preparador.setString(3, usuario.getEndereco().getLogradouro()+" "+usuario.getEndereco().getNome()+" "+usuario.getEndereco().getNumero()+" "+usuario.getEndereco().getComplemento()+" "+usuario.getEndereco().getEstado());
			preparador.setString(4, usuario.getCelular().getTipo()+" "+usuario.getCelular().getDdd()+" "+usuario.getCelular().getNumero());
			preparador.setString(5, usuario.getEmail());
			preparador.setString(6, usuario.getUsername());
			preparador.setString(7, usuario.getSenha());
			preparador.setString(8, usuario.getAcesso());
	...
	public void editar(Usuario usuario) {
		...
		String sql =" UPDATE public.usuario SET nome = ?, cpf= ?, endereco = ?, telefone = ?, email = ?, username = ?, senha = ?, nivel_acesso = ?  WHERE id = ?" ;
		try {
			PreparedStatement preparador = con.prepareStatement(sql);
			preparador.setString(1, usuario.getNome());
			preparador.setString(2, usuario.getCpf());
	...
	public List<Usuario> buscarTodos(Usuario usuario){
		...
		try {
			...
			while(resultados.next()) {
				...
				usu.setCod_usuario(resultados.getInt("id"));
