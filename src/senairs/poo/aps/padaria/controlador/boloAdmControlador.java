...
@WebServlet("/boloAdmControlador")
public class boloAdmControlador extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
   
    public boloAdmControlador() {
        super();
    }
...
//			if(acesso.equals("administrador")) {
				List<Pao> lista = paoDao.buscarTodosBolos(pao);			
				request.setAttribute("lista", lista);
				RequestDispatcher saida = request.getRequestDispatcher("cadastroBolo.jsp");
...
//			}
			...
			paoDao.excluir(pao);			
			response.sendRedirect("boloAdmControlador?acao=lis");
		}
		else if(acao != null && acao.equals("alt")) {
			String id = request.getParameter("id");
			Pao boloatual = paoDao.buscarPaoporID(Integer.parseInt(id));
			request.setAttribute("boloatual", boloatual);
			request.getRequestDispatcher("editacadastroBolo.jsp").forward(request, response);			
		}
